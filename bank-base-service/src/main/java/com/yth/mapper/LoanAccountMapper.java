...
public interface LoanAccountMapper {
    ...
    public List<LoanAccount> findLoanByIdentityNumOrCardNum(@Param("identityNum")String identityNum,@Param("cardNum")String cardNum);
    
    @Select("SELECT * FROM LoanAccount where id=#{id}")
    public List<LoanAccount> findLoanById(@Param("id")int id);
